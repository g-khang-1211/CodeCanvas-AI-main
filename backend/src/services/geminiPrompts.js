export const buildChatPrompt = (context, history, currentMessage, language) => `Context: You are a friendly, expert tutor. The user is currently studying: ${context}.
...
export const buildFlashcardsPrompt = (topic, difficulty, count, language) => `Generate ${count} educational flashcards for the topic: "${topic}". 
...
export const buildSyllabusPrompt = (courseName, level, focus, language) => {
...
export const buildQuizPrompt = (courseName, level, unitTitle, questionCount, questionTypes, language) => `Generate ${questionCount} quiz questions for ${courseName} (${level}) on topic "${unitTitle}".
...
export const buildLessonPrompt = (courseName, level, unitTitle, focus, language) => `Write a comprehensive, engaging educational lesson for ${courseName} (${level}) on the topic: "${unitTitle}".
...
export const buildVideosPrompt = (topic, language) => `Find 3 helpful YouTube video tutorials for the topic: "${topic}".
      Language: ${language}.
